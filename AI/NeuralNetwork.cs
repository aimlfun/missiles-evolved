...
#pragma warning restore CS8618
    ...
    /// <summary>
    /// Performs back-propagation (adjusts biases/weights to get the NN closer to expected result).
    /// </summary>
    /// <param name="inputs">Training data in.</param>
    /// <param name="expected">Training data expected result</param>
    public void BackPropagate(double[] inputs, double[] expected)
    {
        // we are adjusting weights/biases; without feed-forward after doing that, the neuron values will be
        // out of date and back-propagation won't work.
        ...
        // we adjust the hidden layers
        for (int i = Layers.Length - 2; i > 0; i--) 
        {
            layer = i - 1;

            // calculate gamma so we know how much to adjust weights and biases
            for (int j = 0; j < Layers[i]; j++) // outputs
            {
                ...
                gamma[i][j] *= activationDerivativeMethod[layer](Neurons[i][j]); 
            }
    ...
    /// To do so it repeated runs all training data thru the back propagation, adjusting weights and biases.
    /// At a certain point it should be stable, and it runs all training data thru to work out if all sets
    /// produce a close enough answer. If it does it stops training.
    /// </summary>
    /// <param name="trainingData"></param>
    /// <param name="maxError"></param>
    /// <param name="maxAttempts"></param>
    /// <param name="checkAfter">Defer testing the accuracy to improve performance.</param>
    /// <returns></returns>
    internal bool Train(TrainingData[] trainingData, double maxError, int maxAttempts, int checkAfter = 100)
    {
        ...
        for (int i = 0; i < maxAttempts; i++)
        {
            ...
            double deviationDuringLastRun = 0;
            int trainingItemInaccurateInOutputAfterTraining = 0;

            // determine if all of the training data returns an accurate enough output
            foreach (TrainingData data in trainingData)
            {
                double outputFromNeuralNetworkForTrainingData = FeedForward(data.input)[0];
                double deviationFromWhatWeWanted = Math.Abs(outputFromNeuralNetworkForTrainingData - data.output[0]);
                ++trainingItemInaccurateInOutputAfterTraining;
            
                if (deviationFromWhatWeWanted > maxError)
                {
                    deviationDuringLastRun = deviationFromWhatWeWanted;
            ...
            if (trainingComplete)
            {
                // we're done, exit the training
                Debug.WriteLine($"epoch {i}.");
                break;
            }
            else
            {
                // we're not done; every so often output the progress.
                if (i % 1000 == 0) Debug.WriteLine($"epoch {i}. item: {trainingItemInaccurateInOutputAfterTraining} deviation: {deviationDuringLastRun}");
            }
        }

        Debug.WriteLine((trainingComplete) ? "TRAINED" : "** TRAINING FAILURE **");

        // not strictly necessary, but useful. For each input it shows expect result and deviation.
        foreach (TrainingData data in trainingData)
        {
            double result = FeedForward(data.input)[0];
            Debug.WriteLine($"{string.Join(" ", data.input)} = {result:0.###} Deviation from desired value: {Math.Abs(result - data.output[0]):0.###}");
        }
